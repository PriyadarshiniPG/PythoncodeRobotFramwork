...
Force Tags        JIRA_Lukewarm_Standby    PROD-NL-SELENE
...
*** Test Cases ***
I Change the PowerConsumption To LukewarmStandby
    [Documentation]     This step change the power consumption to LukewarmStandby
...
Makes STB To PowerON
    ...
    log to console  stb turn off, wait ${STARTUP_LUKEWARMSTANDBY_DELAY_SECONDS} secs from now: ${now}
    I wait for ${STARTUP_LUKEWARMSTANDBY_DELAY_SECONDS} seconds
    ...
    Wait Until Keyword Succeeds    50s    ${DEFAULT_RETRY_INTERVAL}    Verify Linear TV is Tuned via VLDMS     ${channel_id}
    log action  LukewarmStandby_Done
