...
Force Tags        JIRA_CartoonNetwork    PROD-UK-EOS   PREPROD-UK-EOS
...
Open CatchUP From MainMenu
    ...
    set context     CatchupCartoonNetwork
    ...
    I Press     OK
    Move Focus to Option in Value Picker    textValue:Cartoon Network    DOWN
    I Press     OK
    log action     CatchupCartoonNetwork
    Wait Until Keyword Succeeds    ${DEFAULT_RETRY_TIMEOUT}    ${DEFAULT_RETRY_INTERVAL}
    ...    Catchup Catalog for Cartoon Network is shown
    log action     CatchupCartoonNetwork_Done

Navigate to predefined Asset from Catalog
    [Documentation]    Navigate to the predefined  asset
    [Setup]    Skip If Last Fail
    ${FourthAsset}    Get Title of Tile at Position in Grid    ${0}
    ${FifthAsset}    Get Title of Tile at Position in Grid    ${1}
...
Stop Playback and Validate the Episode Details Page
    ...
    set context     CatchupCartoonNetwork_Back
    I Press    BACK
    log action    OpeningInfoPage
    wait until keyword succeeds    ${DEFAULT_RETRY_TIMEOUT}    ${DEFAULT_RETRY_INTERVAL}    Details Page is Shown
    log action  OpeningInfoPage_Done
